...
object homework_hkt_impllicts{
    ...
    implicit def OptionToBindable[A](opt: Option[A]): Bindable[Option, A] = new Bindable[Option, A] {
      override def flatMap[B](f: A => Option[B]): Option[B] = opt.flatMap(f)
      override def map[B](f: A => B): Option[B] = opt.map(f)
    }
    
    implicit def OptionToBindable[A](list: List[A]): Bindable[List, A] = new Bindable[List, A] {
